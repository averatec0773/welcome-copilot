...
and version as ~10 small functions in a script. I'd flip to one of them if
...
- **Gmail's daily send quota** — the run checks remaining quota before every
  send and stops with 20 sends still in reserve, logging how many rows it
...
- **A crash partway through a send** — the row is stamped `SENDING` and that
  ...
  Outbox and confirm whether the email actually left before clearing it.
